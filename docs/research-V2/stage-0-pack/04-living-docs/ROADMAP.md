...
> **Status: v1.4.2 — two-sided product + confirm-gated AI Broker Copilot (2026-06-17).** All five MVP milestones, the public landing, auth integration, **the charterer Client Portal (`/portal`) + broker Dashboard (`/dashboard`)**, and the broker-only AI Copilot are shipped. Both authenticated homes are role-gated on the `AppUser` identity (Broker → `/dashboard`, Charterer → `/portal`); the landing stays public and now explains the two roles with a Broker / Charterer toggle. 375 unit tests across 60 files; 7 E2E across 4 specs; typecheck and full ESLint are green. Configured for Vercel + Neon deploy. Next: deepen the two-sided workflow and harden role isolation.
...
- [x] Landing unit test coverage updated; full suite: 375 unit tests across 60 files.
