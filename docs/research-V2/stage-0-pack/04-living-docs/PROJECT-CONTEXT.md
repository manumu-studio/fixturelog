...
| Landing page | ✅ `/` — polished public landing page: animated marine-chart hero canvas (vessel tracks, port nodes, laycan arcs, cyan ribbon), Helical motion choreography + maritime editorial skin (navy/cyan), Broker / Charterer role comparison toggle, feature showcase, fleet teaser, how-it-works, tech badges, CTA footer, portfolio disclaimer. `motion@^12` drives the existing animated sections; auth CTAs use the real role-aware sign-in flow. |
