...
<p align="center"><em>v1.4.2 — the public landing now explains the two-sided product with a Broker / Charterer toggle, while the AI Broker Copilot remains grounded in the broker desk and every write stays behind explicit broker approval. Built on the charterer Client Portal <code>/portal</code> + broker Dashboard <code>/dashboard</code>, role-gated on the shared ManuMuStudio OIDC identity.</em></p>
...
|---|---|---|
| `/` | Public | Maritime landing — hero canvas, role comparison toggle, feature showcase, public **Fleet teaser**, role-based sign-in / sign-up CTAs |
...
npm run test                         # 375 unit tests across 60 files
...
src/
  ...
  app/
    page.tsx            # public animated landing (role comparison + role-based CTAs + Fleet teaser)
  ...
  components/
    landing/            # landing sections — each a 4-file component (incl. AuthCta · RoleComparison · FleetTeaser)
...
- **Done (v1.4.2)** — public landing role comparison: a Broker / Charterer toggle explains the `/dashboard` and `/portal` difference before visitors enter the product
...
[ManuMu Studio](https://manumustudio.com). The landing's motion choreography is ported from the author's [Helical Bio Explorer](https://github.com/manumu-studio/helical-bio-explorer); the maritime editorial skin (Fraunces type, deep-navy `#000061` / cyan `#00e2fd`) is informed by a public SSY homepage CSS audit. No SSY brand assets, logos, or trademarks were copied.
