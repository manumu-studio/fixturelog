...
| Shortlist UI | ✅ `/requirements` (polished broker queue with status + screening badges) and `/requirements/[id]` (portal-styled shortlist detail with per-factor breakdown and broker review-gate note) — Next.js 15 server components |
...
| Landing page | ✅ `/` — restored offshore workflow landing: maritime intelligence canvas, proof metrics, feature panels, fleet teaser, workflow steps, tech badges, footer, and auth-aware CTAs. Historical assistant-preview components remain in the repo, but they are not the live public route. `/page2` redirects to `/`. |
...
- **Interview demo surface restoration** — v1.6.2 restores the public product landing after the private-build page, polishes broker requirements/charterers pages, and keeps the AI/risk story bounded: matching, screening, and weather evidence support human review, not vessel clearance or legal/commercial decisions.
