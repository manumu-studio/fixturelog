// Unit tests for GET /api/charterers/:id/fixtures
...
const VALID_FIXTURE = {
  ...
  vesselId: 'clxxxxxxxxxxxxxxxxxxxxxx04',
  requirementId: null,
  laycanStart: null,
  laycanEnd: null,
  freightRate: null,
  ...
  createdAt: new Date(),
  updatedAt: new Date(),
  vessel: { name: 'MV Atlantic', vesselType: 'BULK_CARRIER' },
};
...
describe('GET /api/charterers/:id/fixtures', () => {
  it('returns fixtures with vessel info for a valid charterer ID', async () => {
    ...
    vi.mocked(prisma.fixture.findMany).mockResolvedValue([VALID_FIXTURE]);

    const req = new NextRequest(`http://localhost/api/charterers/${VALID_ID}/fixtures`);
    const res = await GET(req, { params: Promise.resolve({ id: VALID_ID }) });
    const json = await res.json() as { data: Array<{ vessel: { name: string } }> };

    expect(res.status).toBe(200);
    expect(json.data).toHaveLength(1);
    expect(json.data[0]?.vessel.name).toBe('MV Atlantic');
    expect(vi.mocked(prisma.fixture.findMany)).toHaveBeenCalledWith(
  ...
  it('returns 400 for an invalid charterer ID format', async () => {
    const req = new NextRequest('http://localhost/api/charterers/bad-id/fixtures');
